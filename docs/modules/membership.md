...
## 过期会员冻结成绩合同（2026-07-16）

- 日榜和周榜的公开名次仍只包含当前有效会员；过期会员不占名次，也不改变其他用户的排名。
- 已加入榜单但会员过期时，Worker 只在该用户自己的鉴权响应中返回当前周期 `frozenTotalValue`；其他用户无法看到其冻结成绩。
- App 用“我的成绩已冻结”卡片显示该分数，并保留续费和退出榜单入口；续费后由正常有效会员查询恢复排名。
- 复用现有 `leaderboard_daily_totals`，不删除历史聚合、不新增 D1 migration。过期期间训练在续费后是否补算不属于本合同，需另行确定公平性规则。
