...
- 会员成功下载的云端训练历史会复用 `WorkoutSessionStore` 持久化，并按请求开始时捕获的 `appUserId` 隔离；未登录或切换账号后只读取当前 owner 的记录，同一训练 ID 在不同账号下互不冲突。
- 本地云历史只是展示缓存，不授予 Premium 权限，也不改变 Worker、D1 或 `GET /workouts` 的权威合同。只有 `AccountController.premium` 为真时才请求当月云历史；请求成功后后台补充缓存，请求或缓存写入失败都保留旧缓存。
- 同一账号会员失效后不再请求新的云历史，也不上传待同步记录，但此前已缓存的云历史与本地训练仍可只读查看。退出登录不会删除训练文件；owner 过滤负责隐藏其他账号缓存。
- 云端缓存记录统一保存为 `synced`，不会重新进入上传队列；同账号已有本地或待同步记录时以本地版本为准，重复刷新保持幂等。
