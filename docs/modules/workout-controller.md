...
追踪会话在所有终止边界都会关闭：正常 `stop()`、`dispose()`，以及启动/切相机主错误终止会话时（`_cleanupAfterPrimaryError`）。三类路径共用同一个记忆化的 `_closeTraceWhenIdle()` Future，`RecognitionTraceLog.close()` 本身幂等，因此并发或重复调用不会双关闭 sink、不会泄漏 `.jsonl.part`；trace 清理失败不得覆盖主错误或阻止状态收束，只产出固定诊断码与 `error.runtimeType`。
