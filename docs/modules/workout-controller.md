...
追踪会话在所有终止边界都会关闭：正常 `stop()`、`dispose()`、启动/切相机主错误终止会话时（`_cleanupAfterPrimaryError`），以及 `start()` 在 trace 初始化后变 stale 的分支。四类路径共用同一个记忆化的 `_closeTraceWhenIdle()` Future：它先 `await` 记录在 `_traceStart` 的进行中 trace 初始化（`_startTrace` 记录），收束后才执行唯一一次 `_closeTrace()`（内部唯一直接调用 `_trace.close()` 的位置）；`RecognitionTraceLog.close()` 本身幂等，因此并发或重复调用只关闭一次 sink、不泄漏 `.jsonl.part`，也不会因 trace 初始化未完成而过早关闭后再次关闭。trace 清理失败不得覆盖主错误或阻止状态收束，只产出固定诊断码与 `error.runtimeType`。
