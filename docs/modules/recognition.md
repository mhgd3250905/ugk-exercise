...
连续 15 帧（`_maxLostPoseFrames`，约 0.5s）不满足 → 强制退出 ready，保留已完成计数并以当前计数重置动作跟踪，丢弃尚未完成的半次动作。控制器进入持续的 `reacquiringPose` 状态，页面切回对应运动类型的固定准备剪影；只有重新通过窄距/通用准备门控及 ready 深度标定后才恢复运动态。中断提醒只触发一次，文案为“姿势已中断，请按剪影重新准备。”；音频文件 `pose_lost.wav` 待补录，缺失时安全静音。
...
### 异常处理
- 不可用帧 hold 状态，不清零计数
- 异常（举手 / 姿态丢失）达到 15 帧阈值、退回准备态时：**保留已完成计数**，重置未完成动作，保持中断提示并显示对应固定剪影；重新 ready 后继续累计
