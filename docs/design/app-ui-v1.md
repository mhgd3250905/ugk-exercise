...
结构：
- 上方：带底部圆角的相机舞台；未 ready 时显示对应运动类型的固定准备剪影，ready 后切换为实时姿态剪影。
...
- 固定准备剪影采用无五官、无发型、无明显性别特征的粗犷二维任务图标风格，透明度为 30%；构图以头肩手臂为主，横向紧凑，不使用细长全身比例。组件直接受相机舞台尺寸约束：竖向舞台最多占 90% 宽/62% 高，较宽舞台最多占 68% 宽/82% 高。
- 常规与窄距分别使用独立准备剪影；未 ready（含姿态中断后的重新准备）显示固定目标，ready 后只显示实时检测剪影，两种反馈不得同时出现。
...
- 运动态连续 15 帧姿态不可用时强制返回未 ready 界面，已完成计数不清零；教练状态栏持续显示“姿势已中断，请按剪影重新准备。”，直至准备态重新识别通过。
...
- 正式训练相机预览不显示零散的固定头部、肩部或手腕参考点。未 ready 时使用完整的固定目标剪影引导摆姿；ready 后使用实时姿态剪影和状态文案反馈识别状态。
