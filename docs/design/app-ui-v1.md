...
- 启动门完成、首页首帧显示后，App 在后台检查一次 Worker 发布清单；只有清单 `versionCode` 高于安装包且 Google Play 官方更新 API 同时确认当前账号/轨道可更新时，才展示非强制更新弹窗。网络、解析、Play 检查失败或超时均静默放行；用户已离开首页时不再弹窗打断训练等流程。
...
- 2026-07-20 `startup app update prompt`
  启动门后增加 Worker 发布清单与 Google Play 可用性的双重检查；新版本使用主题化非强制弹窗展示中英文更新内容，用户可稍后处理或前往既有 Play 商品页，任何检查失败都不阻塞首页。
