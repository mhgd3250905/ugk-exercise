...
- 能直接访问的 HTTP 接口。
- 原生 Splash 与 Flutter 启动门衔接、首次/回访引导分流，以及相机用途说明的确认和取消。
...
启动与权限真机验收至少覆盖：首次安装三页引导、跳过/完成后重启不再出现、浅/深色冷启动无白屏闪烁、首页先显示本地账号缓存、断网仍可进入首页、点击训练后再申请相机权限，以及允许、拒绝、永久拒绝三种结果。自动化测试不能替代系统权限对话框和 Android 12+ Splash 的真机观察。
...
- 有效会员零分时仍正常排名；修订候选 Worker 部署后，已加入但过期的用户仍以冻结成绩公开排名，只有本人额外看到冻结卡；
- JSON 解析与错误码映射；
