...
**Architecture:** `AccountSessionStore` 在现有安全存储中连同 session 保存最后确认的 `AppUser`；`AccountController.restore()` 先发布缓存身份，再调用 `/me` 和 RevenueCat 刷新权威状态。头像继续使用服务端 URL，但改由持久化图片缓存 provider 加载；缓存资料不授予 Premium 权限，401 时仍清除本地 session。个人卡只在 `restoring` 期间显示小型同步转圈。
...
- `/me` 尚未返回时，`restore()` 已把缓存用户发布为登录态且 `restoring == true`。
- `/me` 返回后用最新用户覆盖缓存，并令 `restoring == false`。
...
- 在 VIP 标记左侧放置 16–18px 的低对比度 `CircularProgressIndicator`，仅在 `controller.restoring` 时出现，并提供本地化语义标签。
