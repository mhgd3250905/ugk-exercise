...
最小实现只修改 Worker：公开日榜/周榜查询所有 `is_joined = 1` 用户，当前用户会员状态继续走权威对账并决定是否返回 `frozenTotalValue`；训练同步的会员门控保持不变。不新增 D1 migration，现有 `0.3.8 (10)` App 已兼容新的响应组合，无需重建 AAB。该修订在获得单独部署授权前仅是本地候选。
