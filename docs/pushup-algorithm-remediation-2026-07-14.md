...
- **播报链路不反向影响算法**：算法计数后立即发起播报；数字音频保留 20ms 前导余量和至少 80ms 尾部缓冲，在训练启动时后台预加载；任何新语音都抢占旧语音，避免快速动作产生播报积压。
...
- 双肩通常仍在画面里并随躯干形成稳定轨迹；用户在最低点自然看向地面时，鼻子可能短暂无法识别。
...
训练态：双肩是最低可信基础，鼻子可靠时参与加权；腕和肘允许离屏
...
| 快速动作仍会漏计 | 三次完整真机日志旧算法回放为 5/8/10；第二、三组的动作空档里鼻或单肩短暂低于 0.3，且用户确认这两组存在漏计 | `motionPoseUsable` 要求鼻和双肩每一点都达到 0.3，先于 counter 丢弃关键底部帧 | 每个核心点保留 0.25 下限，双肩平均仍要求 0.3；举手反证仍按 0.3 | 历史阶段已完成并真机复验；鼻子规则后续继续修正 |
| 标准动作最低点低头仍可能漏计 | 2026-07-19 日志中，旧规则漏记的前两个动作在最低点分别只剩 1 个门控可用深度帧；第一处拒绝仅由鼻子低置信触发，第二处还伴随短暂肩部降置信 | 鼻子被当作运动态 hard-negative，与自然低头场景冲突；低处理帧率和 5 帧中值进一步放大关键帧不足 | 运动态取消鼻子硬门控；逐肩 0.25、双肩平均 0.3 与举手反证保持不变 | 已完成自动化、离线反事实与单组真机复验 |
...
| 数字尾音像被截断 | 18/30 个素材尾部缓冲不足 50ms，6 个不足 20ms；播放器更换数字前会主动停止 | 原素材尾部安全余量不足，连续播放策略允许后一数字中断前一数字 | 原人声波形不动，只追加 80ms 静音；数字播报曾改为串行等待上一段自然结束 | 尾部缓冲保留；串行策略已于 2026-07-19 被最新播报抢占取代 |
| 快速动作时语音落后画面 | 用户完成 15 次时语音仍依次播放 13、14、15 | 数字队列等待每段 WAV 自然结束，累计速度低于动作速度 | 所有新语音立即打断旧语音；英文数字单独使用 1.2 倍速，其他语音保持 1.0 倍速 | 自动化与真机听感均已验证 |
...
2026-07-19 用户确认标准动作最低点会自然低头看向地面。新导出的完整日志显示，旧规则漏记的前两个动作都越过了 50% 相对深度线，但门控后各只剩 1 个可用深度帧，无法让 5 帧中值进入 down 相位；第一处最低点拒绝仅由鼻子置信度不足触发，第二处同时出现鼻子和短暂肩部降置信。

第一性原则上，计数对象是肩部随躯干完成的下压—回升循环，面部朝向不是动作是否有效的反证。因此本轮只取消运动态对鼻子置信度的硬要求：双肩仍须分别达到 0.25 且平均达到 0.3，可见手腕抬高仍直接拒绝。`torsoY` 公式保持不变，鼻子可靠时继续作为低权重或高权重的可选成分，不可靠时由双肩提供信号；准备态仍严格要求鼻、双肩、双腕和双髋可靠可见。

对两组具有明确人工真值的修复前日志做离线反事实回放：原始结果 15/16，取消鼻子硬门控后仍为 15/16，同时恢复 11/4 个原本只因鼻子门控被丢弃的帧，没有新增计数。旧漏记日志没有可靠的总动作真值，因此只用于定位“关键低点帧被丢弃”，不把其中额外候选武断标注为正例或负例。真实 JSONL、人体坐标和设备标识均未进入 Git。

修正包覆盖安装后，用户完成一组真机动作并确认没有漏记。诊断日志记录到 15 个 `down → up` 循环、15 次计数、0 次未计数返回和 0 个帧处理错误；新规则实际接受了 2 个旧鼻子硬门控会拒绝的帧，其中 1 个位于 50% 正式深度线以下。唯一的 lost-pose 发生在最后一次计数约 7.5 秒后，属于用户完成动作后离开支撑姿态，不影响本组结论。该验证仅覆盖单用户、单设备、单组动作，不外推为多机型或多姿态结论。

### 4.9 语音以最新状态为准，不再追赶历史计数

2026-07-19 用户在刻意加快动作时确认画面计数正确，但数字语音因为逐条等待自然结束而落后：动作完成 15 次后仍继续播 13、14、15。第一性原则上，训练语音的职责是及时反馈当前状态，而不是事后完整复述已经过时的每个数字；因此“保住上一条尾音”不能高于“报告最新计数”。

本轮将 guide、ready、count 统一为 latest-wins：新事件使旧事件失效，播放器只串行执行短暂的 stop/play/rate 控制，不再等待 WAV 播放完成。多个异步事件同时到来时用 generation 守卫丢弃过期事件，避免旧调用在 await 后反向覆盖新播报。英文数字按用户要求先试行 1.2 倍速；英文非数字提示和全部中文语音保持 1.0 倍速，并在每次播放时显式设置，避免速度状态残留。音频素材、1—30 上限、预加载和计数算法均不改。
...
flowchart TD
    ...
    F --> G["训练态：双肩提供最低可信基础；鼻子可靠时参与 torsoY"]
...
- 主信号只依赖头肩轨迹和肩置信度。
- 鼻子可靠时参与 `torsoY` 加权；不可见时视为未知，不作为运动态判负依据。
...
- 2026-07-19 最低点鼻子可选修正：`flutter analyze` 0 issue；`flutter test` 565/565；定向测试与硬回放合计 32/32，回放基线保持 step0=5 / v3=5 / v4=3；`git diff --check` 通过。独立六维只读审查最终 P0/P1/P2/P3 均为零。
- 2026-07-19 语音 latest-wins 与英文数字 1.2 倍速：语音定向测试 13/13；`flutter analyze` 0 issue；`flutter test` 568/568；回放基线保持 step0=5 / v3=5 / v4=3。
...
- 2026-07-19 最低点鼻子可选修正包覆盖安装后，用户完成一组动作并确认没有漏记；日志为 15 个完整 `down → up`、15 次计数、0 次未计数返回、0 个帧处理错误。新规则实际接住 2 个旧鼻子硬门控会丢弃的帧；lost-pose 只发生在最后一次计数约 7.5 秒后离开支撑姿态时。该结果仅覆盖单用户、单设备、单组动作。
- 2026-07-19 语音 latest-wins 与英文数字 1.2 倍速 Debug 包已覆盖安装并启动，版本保持 0.3.10+13，未卸载、未清数据；用户完成英文快速动作试听后反馈“测试通过，没有什么问题”。
...
7. **极快计数会牺牲上一条语音的尾音。** 最新语音会立即抢占旧语音，因此极快动作时可能只听到最新数字；这是为了避免语音落后画面而明确选择的产品取舍，不影响画面计数和训练记录。
