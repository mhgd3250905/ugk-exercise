# 俯卧撑识别与计数体验整改记录（2026-07-13—2026-07-14）

> 状态：已完成开发、自动化回归和一轮真机验收
>
> 分支：`feat/pushup-algo`
>
> 整改提交范围：`4528827`—`d27321b`
>
> 分支起点：`2f858d1`
>
...
本轮没有推翻“准备态用手腕建立地面基线、运动态用头肩轨迹计数”的核心设计，而是在真实近景约束下补齐了四个问题：
...
- **播报链路不反向影响算法**：算法计数后立即发起播报；数字音频保留 20ms 前导余量，并在训练启动时后台预加载。
...
当前分支起点是 `2f858d1`，而 `main` 在整改期间已继续前进。合并前应由主分支审核者正常 merge/rebase，解决 `WorkoutController` 和文档可能出现的冲突，然后重新运行 `flutter analyze`、`flutter test` 和真机语音检查；不要以本记录代替合并后的验证。
