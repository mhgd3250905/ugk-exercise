...
| Google Play 测试轨道（最后核对 2026-07-16） | Internal `0.3.8 (11)` 已发布；Alpha `0.3.8-closed-1` 审核中 | Internal 发布名称 `0.3.8-internal-3` 已由 Console 独立核对；同一 AAB 已送审 Alpha，但当时仍在快速检查，不能写成已向 Alpha 测试人员发布 |
| 下一候选源码 | `main@02f30b7`；仍为 `0.3.8 (11)` | 已包含启动/榜单视觉、会员刷新进度、记录日期水印卡、版本号与 Play 更新入口；尚未提升 `versionCode`，未构建或上传新 AAB |
| Play 安装验收 | `0.3.8 (11)` USER REPORTED SMOKE PASS | 用户只报告未发现明显问题；真实 Sandbox 续费、逐项整版回归和新版本提示仍需用下一内部测试候选验证 |
...
| 会员单一权威对账 | WORKER PRODUCTION PASS / APP ALPHA REVIEW | RevenueCat subscriber → Worker 权威裁决 → D1 可重建缓存已上线；`0.3.8 (10)` 排行榜核心链路已通过，包含续费即时刷新的 `0.3.8 (11)` 已送审 Alpha，真实 Sandbox 续费回归仍待单独记录 |
...
- 最后核对的内部测试版本是 `0.3.8 (11)`；同一 AAB 的 Alpha 状态最后记录为审核/快速检查中。
- 下一候选源码为 `main@02f30b7`，`pubspec.yaml` 仍是 `0.3.8+11`，尚未生成候选 AAB。
- 下一次内部测试必须先把 `versionCode` 提高到大于 11；版本号、AAB 哈希、签名和上传状态只能在实际构建后补记。
- 相对已发布 `(11)`，下一候选新增：启动与榜单视觉细化、会员对账等待进度、记录页日期水印卡、个人页版本号与 Google Play 更新入口。
...
2026-07-16 `0.3.8 (11)` 内部测试已验证，Alpha 送审中：
...
- Alpha 状态：经用户明确授权，复用同一 `11 (0.3.8)` App Bundle 创建 `0.3.8-closed-1`，发布比例 `100%`，未重新构建或上传另一份产物。2026-07-16 17:36（北京时间）在发布概览提交 1 项封闭式测试 Alpha 变更；Console 明确显示“已将 1 项更改送审”“正在审核中的更改”，并仍在快速检查常见问题，尚不能记为已向 Alpha 测试人员发布。
- 发布边界：自管式发布已关闭，快速检查和 Google 审核通过后会自动开始 Alpha 全面发布；本轮未推进开放式测试或 Production。此前 `0.3.8 (10)` AAB 不包含这些体验优化，不能冒充本版本。
...
| P0 | 准备下一内部测试候选 | `main@02f30b7` 仍使用已被 Play 占用的 `versionCode=11` | 提升到大于 11，基于已提交源码完成全量测试、Worker 测试、签名/权限/哈希核验后再申请上传授权 |
| P1 | 跟踪 `0.3.8-closed-1` Alpha 审核并抽查 | 同一 `0.3.8 (11)` AAB 已送审，当前仍在快速检查/审核；用户只报告内部测试未发现明显问题，未提供逐项验收清单 | Console 显示已向 Alpha 测试人员发布；从封闭测试加入链接安装/更新后，抽查版本、冷启动衔接、登录、排行榜和续费返回即时刷新 |
| P1 | 验证 App 内更新提示 | 侧载 Debug 只能证明版本行可打开 Play 商品页，不能证明官方更新 API 会返回可用更新 | 从 Play 安装旧版，发布更高 `versionCode` 后确认标签出现、商品页正确、覆盖更新成功且数据保留 |
