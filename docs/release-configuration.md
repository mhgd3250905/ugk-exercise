...
| `0.3.12` 发布后独立审查 | CODE REMEDIATION PASS / PLAY RUNTIME PENDING | 独立审查基线 `601d696`；主线程修复提交 `2cef529` 已通过静态分析、Flutter、Worker、回放与 diff 门禁，尚未构建或发布新的 AAB；Play 覆盖更新验收仍由用户执行 |
...
- `0.3.12` 发布后独立审查以 `601d696` 为只读候选，主线程在 `2cef529` 修复跨上海日/周的迟到排行榜响应污染缓存，以及服务端已确认非会员后旧排名短暂重现的问题；该修复尚未进入已发布的 `versionCode=15` AAB。
...
- 审查线程只读核对需求完整性、逻辑正确性、边界情况、代码质量、测试覆盖和实际运行结果；候选基线为 `601d696`，修复提交为 `2cef529`。
- 修复一：迟到的日榜/周榜响应若已经跨过上海日期或周界，不再写入当前快照或首页排名缓存。
- 修复二：恢复本地账号后仅在服务端会员核验尚未完成时允许显示缓存排名；服务端确认非会员后立即隐藏，不再受后续 RevenueCat 配置阶段的通用 busy 状态影响。
- 新增精确边界与兼容测试：窄距腕宽 `1.25` 及 epsilon、提示防抖页面销毁/重建、积分榜拒绝 v1 次数游标、旧 Worker 排行榜响应的安全失败与本地化重试。
- 修复后门禁：`flutter analyze` 0 issue；Flutter `569/569`；Worker `143/143`；回放硬基线 step0=5 / v3=5 / v4=3；`git diff --check` 通过。
