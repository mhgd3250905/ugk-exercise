...
| 内部测试 | INSTALL REPORTED | `0.3.7 (8)` 已安装不等于本轮新增功能验收通过，仍需按下方清单人工回归 |
| 封闭测试 | PASS | 用户报告 `0.3.4-closed-1` 已于 2026-07-14 10:42 在 Google Play 面向 Alpha 测试人员全面发布 |
| Play 安装 | `0.3.7 (8)` USER REPORTED INSTALLED | 用户报告测试机已安装新的内测版本；安装来源、签名属性和功能结果尚未在本会话复核 |
...
| P1 | 验收 `0.3.7 (8)` Play 安装版新增体验 | 用户报告审核通过且测试机已安装，但尚未报告本轮功能回归结果 | 验证启动引导、浅深色首页、记录动效、账号缓存、屏蔽名单、长按举报入口及计数语音尾音 |
