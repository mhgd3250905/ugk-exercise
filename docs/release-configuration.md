...
| Google Play 测试轨道（最后核对 2026-07-19） | Internal `0.3.12 (15)` 与 Alpha `0.3.11 (14)` 均已全面发布 | Play Console 独立核对：`0.3.12-internal-1` 于 2026-07-19 15:55 面向内部测试人员发布；`0.3.11-closed-1` 于 2026-07-19 11:50 面向 Alpha 测试人员发布 |
| 已发布 Internal 源码 | `codex/play-0.3.12-candidate@e6659dd`；`0.3.12 (15)` | 优化实时训练页、启动流、首页排名加载与窄距提示稳定性；核验 AAB 已发布到 Internal |
| 当前 App 主线 | `main@5553576` | 已包含窄距俯卧撑、积分榜、本人分类次数、英文语音、多页面主题表面优化，以及实时训练页、启动流和窄距提示稳定性修复；远端 main 已核对一致 |
| 当前 Internal 发布 | `0.3.12 (15)` 已全面发布 | `codex/play-0.3.12-candidate`；AAB 源提交 `e6659dd`；发布名称 `0.3.12-internal-1`；Play 安装版功能验收待记录 |
| 当前 Alpha 发布 | `0.3.11 (14)` 已全面发布 | 复用 Internal 的同一 `versionCode=14` App Bundle；`0.3.11-closed-1` 于 2026-07-19 11:50 面向 Alpha 测试人员发布 |
| `0.3.12` 发布后独立审查 | ROUND 3 LOCAL PASS / INDEPENDENT REVERIFY PENDING / PLAY RUNTIME PENDING | 独立审查基线 `601d696`；主线程三轮修复提交 `2cef529`、`c92a00d`、`1edcc06` 已通过本地门禁，正等待同一独立线程第四轮复验；尚未构建或发布新的 AAB |
...
| Play 安装验收 | `0.3.11 (14)` USER REPORTED PASS / `0.3.12 (15)` PENDING | 用户报告上一 Internal 测试通过但未提供逐项清单；新 Internal 已发布，仍需从 Play 覆盖更新后验收版本、数据保留与本轮体验改动 |
...
- `0.3.12` 发布后独立审查以 `601d696` 为只读候选；主线程第一轮 `2cef529` 修复首次加载与正常 restore 路径，第二轮 `c92a00d` 继续覆盖分页、批量刷新、身份刷新、旧快照回填，以及 restore 失败后由 refresh 接受会员结论的路径，第三轮 `1edcc06` 统一头像政策后的有效 `/me` 会员状态接收，并隔离跨上海周期迟到失败。第三轮本地门禁通过，独立第四轮复验待完成；这些修复均未进入已发布的 `versionCode=15` AAB。
...
- 审查线程只读核对需求完整性、逻辑正确性、边界情况、代码质量、测试覆盖和实际运行结果；候选基线为 `601d696`，三轮修复提交为 `2cef529`、`c92a00d`、`1edcc06`。
...
- 第三轮补齐头像政策接受后的有效 `/me` 统一结束会员核验 pending，以及日/周首次加载、分页、批量刷新和身份刷新在跨上海周期后迟到失败不污染快照、首页排名、错误或 loading 状态；同时保留当前周期失败的既有错误语义。新增七个精确回归测试，本地门禁为 `flutter analyze` 0 issue、Flutter `581/581`、Worker `143/143`、回放 `31/31` 且硬基线 step0=5 / v3=5 / v4=3、`git diff --check` 通过。
- 当前状态是“第三轮本地通过、同一独立审查线程第四轮复验待完成”，不得提前记为独立审查通过。
