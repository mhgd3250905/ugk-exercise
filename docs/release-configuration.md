...
| Google Play 测试轨道（最后核对 2026-07-17） | Internal `0.3.9 (12)` 已发布；Alpha `0.3.8-closed-1` 已发布 | 用户报告 `0.3.9-internal-1` 于 2026-07-17 09:53 面向内部测试人员发布；Alpha 状态已由本会话独立核对 |
| 已发布 Internal 源码 | `codex/play-0.3.9-candidate@3bb285b`；`0.3.9 (12)` | 已包含启动/榜单视觉、会员刷新进度、记录日期水印卡、版本号与 Play 更新入口；同一核验 AAB 已发布到 Internal |
| 下一 Internal 候选 | `codex/play-0.3.10-candidate@00ac78c`；`0.3.10 (13)` | 修复版本入口跳转并统一账号/会员全局状态；签名 AAB 已在本地完成核验，尚未上传或创建 Play 发布 |
...
- 用户报告的当前内部测试版本是 `0.3.9 (12)`；发布名称 `0.3.9-internal-1` 于 2026-07-17 09:53 面向内部测试人员发布。
- 当前 Internal 源码为 `codex/play-0.3.9-candidate@3bb285b`；发布产物与下方已核验 AAB 完全一致。
- Alpha 当前仍为 `0.3.8 (11)`，已于 2026-07-16 17:54 面向测试人员全面发布；本次未推进 Alpha、开放测试或 Production。
- 相对 `0.3.8 (11)`，`0.3.9 (12)` 新增：启动与榜单视觉细化、会员对账等待进度、记录页日期水印卡、个人页版本号与 Google Play 更新入口。
...
2026-07-17 `0.3.10 (13)` Google Play 内部测试候选（本地已核验，尚未上传）：
...
- 计划发布名称：`0.3.10-internal-1`。当前仅完成本地构建与核验；未上传 AAB，未创建发布，未推进 Internal、Alpha、开放测试或 Production。
...
| P1 | 抽查 `0.3.8-closed-1` Alpha | Console 已确认同一 `0.3.8 (11)` AAB 面向 Alpha 测试人员全面发布；尚未记录从封闭测试加入链接安装后的人工抽查 | 抽查版本、冷启动衔接、登录、排行榜和续费返回即时刷新 |
| P1 | 复验 App 内更新入口 | `0.3.9 (12)` 的版本入口曾把包名交给浏览器搜索；`0.3.10 (13)` 已修复但尚未上传 | 从 Play 安装 `0.3.10-internal-1`，确认直接打开正确商品页、覆盖更新成功且数据保留 |
