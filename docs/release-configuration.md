...
| 内部测试已发布版本 | `0.3.8 (11)` USER REPORTED PUBLISHED | 用户报告已在 Google Play 提交并面向内部测试人员发布；发布名称、精确时间和 Console 页面尚未独立核对 |
...
| 内部测试 | `0.3.8 (11)` PUBLISHED / PLAY INSTALL PENDING | 发布状态来自用户报告；尚未记录从 Play 覆盖更新及本候选完整回归 |
...
2026-07-16 `0.3.8 (11)` 内部测试发布（用户报告已提交并发布）：
...
- 状态：用户报告本 AAB 已提交并面向内部测试人员发布；发布名称、精确时间和 Console 页面尚未独立核对。从 Play 覆盖更新、真实 Sandbox 续费及整版回归尚未记录完成；未报告推进封闭测试、Alpha 或 Production。此前 `0.3.8 (10)` AAB 不包含这些优化，不能作为本版本复用。
...
| P1 | 验收 `0.3.8 (11)` Play 安装版 | 用户报告 `versionCode=11` 已面向内部测试人员发布；本地证据和发布状态不能替代 Play App Signing 后的覆盖更新与 Billing 真机链路 | 从 Play 覆盖更新，验证冷启动衔接、续费返回即时刷新和主要回归项，再单独决定是否推进封闭测试 |
