...
| 内部测试已发布版本 | `0.3.8 (10)` CONSOLE VERIFIED PUBLISHED | Play Console 已独立核对：发布名称 `0.3.8-internal-2` 于 2026-07-16 11:18（北京时间）面向内部测试人员发布，包含 1 个版本代码 |
| 下一内部测试候选 | `0.3.8 (11)` SOURCE READY / AAB PENDING | 最新体验优化已提交并通过本地回归；尚未构建、核验或上传 release AAB，不得复用 `versionCode=10` 的旧产物 |
| 内部测试 | `0.3.8 (10)` PUBLISHED / USER UPDATED | 用户已从 Play 更新并进入运动广场；完整候选回归仍未记录完成 |
| 封闭测试 | PASS | 用户报告 `0.3.4-closed-1` 已于 2026-07-14 10:42 在 Google Play 面向 Alpha 测试人员全面发布 |
| Play 安装 | `0.3.8 (10)` USER REPORTED LEADERBOARD PASS | 用户报告修订 Worker 部署后，过期账号已重新出现在公开榜单且本人冻结卡仍保留；其余完整候选回归仍待完成 |
...
| 会员单一权威对账 | WORKER PRODUCTION PASS / APP INTERNAL PUBLISHED | RevenueCat subscriber → Worker 权威裁决 → D1 可重建缓存已上线；`0.3.8 (10)` 已从 Play 更新并进入运动广场，完整对账回归仍待完成 |
| 当前设备验收 | PLAY 0.3.8 LEADERBOARD PASS / DEBUG UI PASS | 用户报告 `0.3.8 (10)` 已验证过期账号公开排名与本人冻结卡；最新 Debug 已验证首页卡片和启动视觉，续费后榜单即时刷新仍只有自动化证据，待下一 Play 候选真机回归 |
...
2026-07-16 `0.3.8 (11)` 下一内部测试候选（源码已保存，AAB 待构建）：

- 功能提交：`eef876b`；版本提交：`e9feee8`；分支仍为 `codex/play-0.3.8-candidate`。
...
- 状态：`versionName=0.3.8`、`versionCode=11` 已写入源码；release AAB 尚未构建、签名核验或上传。此前 `0.3.8 (10)` AAB 不包含这些优化，不能作为本候选复用。
...
| P1 | 构建并验收 `0.3.8 (11)` Play 安装版 | `0.3.8 (10)` 已发布但不含最新启动、首页边界和续费后即时刷新优化；`(11)` 的 Flutter/Worker 本地回归已通过，尚无 release AAB | 构建并核验 `versionCode=11` AAB；获得单独上传授权后走内部测试，验证覆盖更新、冷启动衔接、续费返回即时刷新和主要回归项 |
