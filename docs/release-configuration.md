...
| 内部测试已发布版本 | `0.3.8 (9)` USER REPORTED PUBLISHED | 用户报告发布名称 `0.3.8-internal-1` 已面向内部测试人员发布，包含 1 个版本代码；Console 显示发布时间为 2026-07-16 10:05，本会话未独立读取 Console 证据 |
| 内部测试 | `0.3.8` PUBLISHED / PLAY INSTALL PENDING（用户报告） | 发布状态已记录；尚未报告从 Play 安装、覆盖更新或本候选真机功能验收 |
...
| 会员单一权威对账 | WORKER PRODUCTION PASS / APP INTERNAL PUBLISHED | RevenueCat subscriber → Worker 权威裁决 → D1 可重建缓存已上线；包含 Flutter 强制对账语义的 `0.3.8 (9)` 已由用户报告发布到内部测试，尚未经 Play 安装验收 |
