...
| Google Play 测试轨道（最后核对 2026-07-19） | Internal `0.3.11 (14)` 与 Alpha `0.3.10 (13)` 均已全面发布 | Play Console 只读核对：`0.3.11-internal-1` 于 2026-07-19 11:10 面向内部测试人员发布；`0.3.10-closed-1` 于 2026-07-17 14:42 面向 Alpha 测试人员发布 |
| 已发布 Internal 源码 | `codex/play-0.3.11-candidate@99065aa`；`0.3.11 (14)` | 新增窄距俯卧撑、积分榜与本人分类次数、英文训练语音及多页面主题表面优化；核验 AAB 已发布到 Internal |
| 当前 App 主线 | `main@5553576` | 已包含窄距俯卧撑、积分榜、本人分类次数、英文语音、多页面主题表面优化，以及实时训练页、启动流和窄距提示稳定性修复；远端 main 已核对一致 |
| 当前 Internal 发布 | `0.3.11 (14)` 已全面发布 | `codex/play-0.3.11-candidate`；AAB 源提交 `99065aa`；发布名称 `0.3.11-internal-1`；Play 安装版功能验收待记录 |
| 下一 Internal 候选 | `0.3.12 (15)` 已构建并完成本地核验 | `codex/play-0.3.12-candidate@e6659dd`；实时训练页、启动流、首页排名加载和窄距提示稳定性优化；尚未上传 Play |
| Alpha 审核中 | `0.3.11-closed-1` 已提交审核 | 复用同一 `versionCode=14` App Bundle，100% 发布；Play Console 独立核对为“正在审核中的更改”，尚未向 Alpha 测试人员发布 |
...
| Play 安装验收 | `0.3.11 (14)` USER REPORTED PASS | 用户报告 Internal 测试通过，但未提供逐项清单；不据此扩写未明确列出的完整场景 |
...
- 用户报告审核通过；本会话随后于 2026-07-19 独立只读核对 Play Console：当前 Internal 为 `0.3.11-internal-1`，`versionCode=14`，已于 11:10（北京时间）面向内部测试人员全面发布，App Bundle 状态有效。
- 当前 Alpha 为 `0.3.10-closed-1`，复用同一 `versionCode=13` AAB，已于 2026-07-17 14:42 面向 Alpha 测试人员全面发布。
...
- 经用户明确授权，本会话复用内容库中的同一 `versionCode=14` App Bundle 创建 Alpha 草稿 `0.3.11-closed-1`，发布比例 100%，中英文版本说明已保存，设备支持范围相对上一 Alpha 无减少。
- 用户报告已提交审核；本会话随后独立只读核对 Play Console：`0.3.11-closed-1` 的“开始全面发布”变更已进入“正在审核中的更改”。当前已发布 Alpha 仍为 `0.3.10-closed-1`（`versionCode=13`），不得提前记为版本 14 已向测试人员发布。未推进开放测试或 Production。
- 下一 Internal 候选为 `0.3.12 (15)`，源自 `main@5553576`，当前仅完成签名 AAB 构建与本地核验，尚未上传或发布。
...
- Alpha 审核状态：用户报告已提交审核；本会话随后独立只读核对 Play Console，`0.3.11-closed-1` 的“开始全面发布”变更已进入“正在审核中的更改”。当前已发布 Alpha 仍为 `0.3.10-closed-1`（`versionCode=13`），不得提前记为版本 14 已向测试人员发布。
- 发布边界：本轮未重新上传 AAB，未推进开放测试或 Production，未部署 Worker、未写 D1、未修改任何平台配置。

2026-07-19 `0.3.12 (15)` Google Play 内部测试候选（已构建，尚未上传）：
...
- 发布状态：本地候选已构建并核验，尚未上传 Google Play，未创建 Internal 发布，未推进 Alpha、开放测试或 Production。
