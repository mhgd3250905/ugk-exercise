...
| 内部测试已发布版本 | `0.3.8 (11)` CONSOLE VERIFIED PUBLISHED | Play Console 已独立核对：发布名称 `0.3.8-internal-3` 于 2026-07-16 16:54（北京时间）面向内部测试人员发布，包含 1 个版本代码（11） |
| 上一内部测试版本 | `0.3.8 (10)` CONSOLE VERIFIED PUBLISHED | Play Console 已独立核对：发布名称 `0.3.8-internal-2` 于 2026-07-16 11:18（北京时间）面向内部测试人员发布，包含 1 个版本代码 |
| 内部测试 | `0.3.8 (11)` PUBLISHED / USER REPORTED SMOKE PASS | 用户报告本版本测试未发现明显问题；未提供逐项验收清单，不能扩写为真实 Sandbox 续费或整版完整回归均已通过 |
| 封闭测试 | `0.3.8-closed-1` REVIEW / FAST CHECK | 经用户明确授权复用同一 `0.3.8 (11)` AAB 推进 Alpha；Play Console 已显示“已将 1 项更改送审”“正在审核中的更改”，快速检查仍在进行，尚不能记为已发布 |
| Play 安装 | `0.3.8 (11)` USER REPORTED SMOKE PASS | 用户报告测试未发现明显问题；`0.3.8 (10)` 的过期账号公开排名与本人冻结卡此前已通过，`0.3.8 (11)` 的真实 Sandbox 续费和逐项整版回归仍未单独记录 |
...
| 当前设备验收 | PLAY 0.3.8 USER REPORTED SMOKE PASS | 用户报告 `0.3.8 (11)` 测试未发现明显问题；已知 `0.3.8 (10)` 的过期账号公开排名与本人冻结卡及最新 Debug 的首页/启动视觉通过，未将未列明项目扩写为完整通过 |
