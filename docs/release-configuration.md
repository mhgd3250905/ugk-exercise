...
| 内部测试 | `0.3.8 (10)` PUBLISHED / PLAY INSTALL PENDING | Worker 已先行部署并通过未登录鉴权探针；尚未报告从 Play 安装、覆盖更新或本候选真机功能验收 |
...
| 会员单一权威对账 | WORKER PRODUCTION PASS / APP INTERNAL PUBLISHED | RevenueCat subscriber → Worker 权威裁决 → D1 可重建缓存已上线；包含 Flutter 强制对账语义的 `0.3.8 (9)` 和后续 `0.3.8 (10)` 均已发布到内部测试，最新版本尚未经 Play 安装验收 |
...
- 尚未报告从 Play 安装、覆盖更新、引导页关闭动画回归或过期会员本人冻结成绩真机验收；未推进封闭测试、Alpha 或 Production。
- 用户随后从 Play 安装验证首版冻结规则，并明确修订为“加入后只有主动退出才移除；会员过期后公开保留冻结成绩并继续参与排序”。该修订已在本地 Worker 与测试中完成，尚未部署；现有 `0.3.8 (10)` App 可直接兼容，后续只需单独授权 Worker 部署，不需要重建或上传 AAB。
...
| P1 | 验收 `0.3.8 (10)` Play 安装版 | Console 已确认内部测试发布，尚无安装与功能回归证据 | 验证覆盖更新、关闭系统动画后的引导前进、过期会员本人冻结成绩、记录动效、屏蔽/举报入口及计数语音尾音 |
