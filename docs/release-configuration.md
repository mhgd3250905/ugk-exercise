...
| 内部测试已发布版本 | `0.3.6 (7)` USER REPORTED PUBLISHED | 用户报告 `0.3.6-internal-1` 已于 2026-07-15 09:14（北京时间）面向内部测试人员发布；已完成 Play 覆盖更新与当时源码主链路验收 |
| 内部测试新候选 | `0.3.7 (8)` BUILT / NOT UPLOADED | 基于已合入功能分支的 main 构建；签名、版本、清单、权限、哈希和自动化已核验，等待用户单独授权上传 |
...
2026-07-15 `0.3.7 (8)` 内部测试候选（已构建，未上传）：
...
- 状态严格为 **已构建、未上传**。计数语音尾音修正尚待真机听感复验；Google Play UGC、Data safety 与内容分级增量声明尚未记录完成，因此本候选只准备进入内部测试，不得直接推进 Alpha 或 Production。
...
| P1 | 经单独授权上传 `0.3.7 (8)` 内部测试候选 | 候选已完成构建与本地核验，但 Google Play 尚无本次远端写入授权 | 上传同一 SHA-256 的 AAB，记录发布名称和 Play 实际状态；不重新构建 |
