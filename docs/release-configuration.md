...
| 封闭测试 | `0.3.3 (4)` Alpha 候选已构建 | `0.3.2 (3)` 已提交审核，尚未确认获批；`0.3.3 (4)` 签名 AAB 已校验但未上传；需至少 12 人连续参与 14 天 |
...
2026-07-13 Alpha 候选产物（已构建验证，未上传）：
...
- release 合并清单不包含 `READ_MEDIA_IMAGES`、`READ_MEDIA_VIDEO` 或 `AD_ID`
- 当前只是本机候选包；上传 Google Play 前需用户再次授权，且需在 Play Console 确认 `0.3.2 (3)` 的审核/发布状态。
