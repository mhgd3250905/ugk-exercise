...
| 待上传候选 | `0.3.6 (7)` BUILT | 自定义头像与治理候选已完成测试、签名和清单核验；尚未上传 Google Play，仅允许先走内部测试 |
...
2026-07-15 `0.3.6 (7)` 内部测试候选产物（已构建，尚未上传）：
...
- 状态：尚未上传、未发布、未从 Play 安装。Google Play 的 UGC、Data safety 和内容分级声明尚未记录为完成，因此只能先走内部测试，不得直接推进 Alpha 或 Production。
