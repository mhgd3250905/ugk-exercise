...
| 内部测试 | 已发布 | 用户报告 `0.3.4-internal-1`（`versionCode=5`）已于 2026-07-14 09:59 面向内部测试人员发布，待 Play 安装真机验收 |
| 封闭测试 | 已发布 | `0.3.3-closed-1` 已面向 Alpha 测试人员发布；`0.3.4 (5)` 候选尚未上传；当前 12 名测试人员已选择参与 1 天 |
...
2026-07-14 Alpha 候选产物（已完成内部测试发布，待真机验收）：
...
- JAR 签名完整，并与已记录的 Google Play 上传证书匹配
- release 合并清单不包含 `READ_MEDIA_IMAGES`、`READ_MEDIA_VIDEO` 或 `AD_ID`
- 候选分支已推送；用户报告同一 AAB 已作为 `0.3.4-internal-1` 于 2026-07-14 09:59 面向内部测试人员发布，尚未推进 Alpha，待从 Google Play 覆盖更新并完成真机验收。
