...
| 内部测试新版本 | `0.3.6 (7)` USER REPORTED PUBLISHED | 用户报告 `0.3.6-internal-1` 已于 2026-07-15 09:14（北京时间）面向内部测试人员发布；已完成 Play 覆盖更新与当时源码主链路验收；后续功能只在 production Debug 验收，不属于该 AAB |
| 内部测试 | PASS | `0.3.6 (7)` 已从 Google Play 覆盖更新并启动；安装器为 `com.android.vending`，包不可调试 |
| 封闭测试 | PASS | 用户报告 `0.3.4-closed-1` 已于 2026-07-14 10:42 在 Google Play 面向 Alpha 测试人员全面发布 |
| Play 安装 | `0.3.6 (7)` PASS | 已确认版本、Play 安装来源、非 Debug、启动、登录态、自定义头像和运动广场状态保留 |
...
- 发布边界：上述 Play AAB 固定对应 `a88ee5a`。其后加入的屏蔽名单、启动引导、首页视觉、记录切换动效、交互确认和长按举报/屏蔽入口均不在 `0.3.6 (7)` 中；这些能力后来只在带 production 配置的 Debug 模拟器完成验收。当前 `pubspec` 仍是 `0.3.6+7`，不得用同一 versionCode 重新上传；下一次 Play 候选必须在 main 审核通过后提高版本号/代码并重新走完整构建 SOP。
...
| P1 | main 审核并集成当前功能分支 | 当前分支相对 main 包含启动引导、首页/记录/个人交互、屏蔽名单和举报入口等一组新能力 | main 审核报告无阻断项，合并后重新运行 Flutter/Worker 全量测试 |
| P1 | 创建高于 `0.3.6 (7)` 的内部测试候选 | 当前分支新功能不在已发布 AAB 中，且 Play 不接受重复 versionCode | 明确新版本号，基于已提交 main 构建签名 AAB，完成签名、权限、哈希和台账核验后再单独授权上传 |
| P1 | 从 Play 安装新版并验收本轮新增体验 | 当前人工验收使用 production Debug，只证明功能与生产 API 合同可用 | 覆盖安装新版 Play 包，验证启动引导、浅深色首页、记录动效、账号缓存、屏蔽名单及长按举报入口 |
