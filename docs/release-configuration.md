...
| 下一内部测试候选 | `0.3.8 (11)` AAB VERIFIED / NOT UPLOADED | 最新体验优化已提交并通过 Flutter/Worker 回归，release AAB 的签名、证书、版本、SDK、权限和哈希均已核验；尚未上传 Google Play |
...
2026-07-16 `0.3.8 (11)` 下一内部测试候选（AAB 已构建核验，尚未上传）：
...
- 状态：AAB 已构建并完成本地核验，尚未上传、创建发布或推进任何轨道。此前 `0.3.8 (10)` AAB 不包含这些优化，不能作为本候选复用。
...
| P1 | 上传并验收 `0.3.8 (11)` Play 安装版 | `versionCode=11` AAB 已完成本地核验但尚未上传；本地证据不能替代 Play App Signing、覆盖更新和 Billing 真机链路 | 获得单独上传授权后走内部测试，验证覆盖更新、冷启动衔接、续费返回即时刷新和主要回归项，再单独决定是否推进封闭测试 |
