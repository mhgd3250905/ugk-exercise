...
| 内部测试 | USER REPORTED PUBLISHED | 用户报告 `0.3.5 (6)` 已上传并发布到内部测试；本轮未重新打开 Play Console 独立核实轨道状态 |
| 封闭测试 | PASS | 用户报告 `0.3.4-closed-1` 已于 2026-07-14 10:42 在 Google Play 面向 Alpha 测试人员全面发布 |
| Play 安装 | `0.3.4 (5)` 已验证；`0.3.5 (6)` 待冒烟 | 本轮购买设备经签名核对为侧载 Debug `0.3.5 (6)`，不是 Play App Signing 分发包 |
...
| 当前设备验收 | 会员 Debug Sandbox PASS | 侧载 Debug `0.3.5 (6)` 已验证月度过期后恢复非会员、年度购买后重启仍保持会员；Play 安装版 `0.3.5` 待冒烟 |
...
2026-07-14 `0.3.5 (6)` 内部测试候选产物（已构建；用户报告已发布到内部测试）：
...
- 状态：用户报告同一候选已上传并发布到内部测试；本轮未独立核实 Play Console。月度/年度 Sandbox 购买已在侧载 Debug `0.3.5 (6)` 通过，不得误记为 Play App Signing 安装包购买通过。
...
| Google Play 订阅购买 | PASS（License Tester Debug） | 侧载 Debug `0.3.5 (6)` 的月度购买/续订/过期、年度购买、RevenueCat Sandbox entitlement、App 重启恢复及 Webhook→D1 均通过；Play App Signing `0.3.5` 待冒烟 |
...
| P1 | Play 安装版 `0.3.5 (6)` 冒烟 | 本轮完整购买发生在 Google 官方支持的 License Tester Debug 包，尚未证明本候选的 Play App Signing 安装态 | 从内部测试链接安装 `0.3.5 (6)`，确认版本、Google 登录、两档套餐加载及已有 Sandbox 订阅恢复；无需再次购买 |
...
- 正式订阅、base plan、License Testing 名单和 RevenueCat Product → `premium` → Package → Offering 映射已完成；侧载 Debug 的月度/年度 Sandbox 购买与 Webhook→D1 已于 2026-07-15 验证通过，Play App Signing `0.3.5` 冒烟仍待执行。
