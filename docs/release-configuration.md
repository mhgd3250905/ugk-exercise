...
| Google Play 测试轨道（最后核对 2026-07-17） | Internal `0.3.8 (11)` 已发布；Alpha `0.3.8-closed-1` 已发布 | Console 已独立核对：Internal 于 2026-07-16 16:54、Alpha 于 2026-07-16 17:54 面向对应测试人员全面发布 |
| 下一候选源码 | `codex/play-0.3.9-candidate@3bb285b`；`0.3.9 (12)` | 已包含启动/榜单视觉、会员刷新进度、记录日期水印卡、版本号与 Play 更新入口；AAB 已构建并通过发布门禁，尚未上传 |
...
- 最后核对的内部测试版本是 `0.3.8 (11)`；同一 AAB 的 Alpha 已于 2026-07-16 17:54 面向测试人员全面发布。
- 下一候选源码为 `codex/play-0.3.9-candidate@3bb285b`，版本为 `0.3.9 (12)`；AAB 已构建并核验，尚未上传。
- Play Console 最后核对的最高版本代码仍为 11；候选 `versionCode=12` 满足递增要求。
...
2026-07-17 `0.3.9 (12)` Google Play 内部测试候选（已构建并核验，尚未上传）：
...
- 用途：上传 Google Play 内部测试，验证 App 内更新标签、商品页跳转、覆盖更新和数据保留；当前状态仅为已构建，不能写成已上传或已发布。
...
| P0 | 发布 `0.3.9 (12)` 内部测试候选 | AAB 已完成全量测试、签名、权限和哈希核验，尚未上传 | 只上传本次记录的同一 AAB；Console 明确显示已向内部测试人员发布，并记录发布名称与时间 |
