...
Git 基线最后核对：2026-07-24（`main@5f20e0d`，`pubspec.yaml` 为 `0.3.21+24`）

平台状态最后核对：2026-07-23（后续变化需 Play Console 核对）
...
| Google Play 测试轨道（最后核对 2026-07-23） | Internal `0.3.20 (23)` 已全面发布；Alpha `0.3.19 (22)` 已全面发布；Alpha `0.3.20 (23)` 审核中 | Play Console 独立核对：`0.3.20-internal-1` 于 2026-07-23 12:18 面向内部测试人员发布；`0.3.19-closed-1` 已审核通过并面向 Alpha 测试人员全面发布；`0.3.20-closed-1` 于 2026-07-23 提交送审，审核通过前不算已发布 |
| 已发布 Internal 源码 | `main@36ce274`（三分支合并）→ `codex/play-0.3.20-candidate@b8db7f5`；`0.3.20 (23)` | 近距离 ready 阻断 + PR#13 三 bug 修复 + 排行榜 UI 优化 + 0.3.20 版本号；唯一核验 AAB 已发布到 Internal |
| 当前 App 主线（Git 核对 2026-07-24） | `main@5f20e0d`；`pubspec.yaml` 为 `0.3.21+24` | 这是 App 仓库事实，不代表 `0.3.21 (24)` 已构建、上传或进入任何 Play 轨道 |
| 当前 Internal 发布 | `0.3.20 (23)` 已全面发布 | AAB 源提交 `b8db7f5`；发布名称 `0.3.20-internal-1`；Play 已确认 `versionCode=23` 于 2026-07-23 12:18 面向内部测试人员发布 |
| 下一 Internal 候选 | App 仓库未核实已构建候选 | 源码版本为 `0.3.21+24`；是否已在 Play 使用 `versionCode=24` 需 Play Console 核对，后续候选必须使用更高且未被 Play 使用的 `versionCode` |
| 当前 Alpha 发布 | `0.3.19 (22)` 已全面发布；`0.3.20 (23)` 审核中 | `0.3.19-closed-1` 已审核通过并面向 Alpha 测试人员全面发布；`0.3.20-closed-1` 于 2026-07-23 复用 Internal 同一 AAB（versionCode 23）从内容库推进 Alpha 并提交送审，尚未发布 |
| 最新 Internal 功能版 | `0.3.20 (23)` 已全面发布 | `codex/play-0.3.20-candidate@b8db7f5`；AAB `168471142` 字节，SHA-256 `5c883d50ef20161f9cb4c7a23ba80193e75c1d4d6f033f491e5a05a2370a62e0` |
...
| Cloudflare Worker/D1 | APP UPDATE `0.3.20 (23)` PRODUCTION DEPLOYED + ADMIN CSRF LIVE / 会员 Webhook→D1 PASS | 2026-07-23 从 `codex/play-0.3.20-candidate@b8db7f5` 部署 0.3.20 中英文更新清单（versionCode 22→23，无回退）；管理台 POST 仍需 Access JWT + Origin + actor 绑定 CSRF token 三层校验；部署后 6 项探针全过，未执行 D1 migration、未写 D1、未改 Secret/变量/binding。精确 Version ID 只在本机私密台账 |
...
### 6.3 2026-07-21 历史发布记录（非当前摘要）

> 本节保留 2026-07-21 及更早发布证据，仅供审计，不再维护“当前”状态。当前 Git 基线与平台最后核对状态只看 [§1 当前状态摘要](#1-当前状态摘要)；任何新上传或轨道推进前仍须在 Play Console 重新核对。

截至 2026-07-21 的发布事实：

- 本会话于 2026-07-21 独立发布并核对：当时 Internal 为 `0.3.16-internal-1`，`versionCode=19`，已于 03:22（北京时间）面向内部测试人员发布。
- 当时 Alpha 为 `0.3.14-closed-1`，复用 `versionCode=17` AAB，已于 2026-07-20 14:28 面向 Alpha 测试人员全面发布。
...
- `0.3.14 (17)` 的已上传 AAB 基于当时已同步的 `main@6774dc8`，候选分支为 `codex/play-0.3.14-candidate`，AAB 源提交为 `7f84e58ef3515b67254dbb19088084b5219e4220`。版本包含月卡 3 天与年卡 7 天试用卡片、精确 Offering 条款识别、无资格安全回退和 Google Play 订阅管理入口；不包含 Worker、D1 或 API 合同变更。AAB 发布后候选分支已无冲突变基到 `main@11d6d2b`，不改变上述构建来源事实。
- 候选门禁：`flutter analyze` 0 issue、Flutter `616/616`、Worker `143/143`、回放 `26/26` 且硬基线 step0=5 / v3=5 / v4=3、`git diff --check` 通过。AAB 路径为 `build/app/outputs/bundle/release/app-release.aab`，大小 `187181119` 字节，SHA-256 `79A782574A6B941C3ACFFD538012630436D370B1BA94F592BACFE79669495E62`；JAR 签名完整，上传证书与私密台账一致，包名 `com.ugkexercise.ugk_exercise`，`minSdk=24`、`targetSdk=35`，Release 不可调试，包含 Billing/Internet 且不包含媒体、外部存储或 `AD_ID` 权限。
- 2026-07-20 经用户明确授权，将上述同一核验 AAB 上传并发布为 Internal `0.3.14-internal-1`。Play Console 预览确认 `17 (0.3.14)`、中英文版本说明及各设备类型不再支持数均为 0；11:42（北京时间）显示“已面向内部测试人员发布”。随后已独立核对模拟器安装版为 `0.3.14 (17)`、安装器为 `com.android.vending` 且 Release 不可调试；截图确认月卡 3 天与年卡 7 天卡片和转正价格均正常显示，用户报告本次测试通过。本轮没有发起年卡购买，因此不能扩写为年卡结算、Webhook、Worker 或 D1 运行链路通过。
- 同日用户报告已复用该 `versionCode=17` AAB 创建 Alpha `0.3.14-closed-1` 并提交审核；后续已独立核对该版本于 2026-07-20 14:28 面向 Alpha 测试人员全面发布。本轮未推进开放测试或 Production。
- `0.3.15 (18)` 的 AAB 源提交为 `36a987edbd1dcf3f534321ca246d2d80533d4b52`，发布名称 `0.3.15-internal-1`；Play Console 已独立核对该版本于 2026-07-20 18:15 面向内部测试人员全面发布。生产 Worker 仍广告 `0.3.14 (17)`，因此不会向 `0.3.15` 误弹更新提示。
- `0.3.16 (19)` 候选来自已合入 `main@c79b14e` 的 `codex/workout-history-cache@8321c25`，候选分支为 `codex/play-0.3.16-candidate`，AAB 源提交为 `1e47a080588414e7af6623f13902dfd910fb9ea6`。候选包含训练历史本地缓存、会员到期后保留已下载历史但停止云端刷新、训练状态提示条宽度/居中/运行色优化，以及会员到期续订核验修复。
- 候选门禁：`flutter analyze` 0 issue、Flutter `658/658`、Worker `149/149`、硬基线 step0=5 / v3=5 / v4=3、`git diff --check` 通过。AAB 路径为 `build/app/outputs/bundle/release/app-release.aab`，大小 `187207830` 字节，SHA-256 `4C2E23654BB558BF94A920C2B6DD9CB98D1E935967B1B2D6C971C9AD3F25F614`；JAR 签名完整，上传证书与私密台账一致，包名 `com.ugkexercise.ugk_exercise`，`minSdk=24`、`targetSdk=35`，Release 不可调试，不包含 `READ_MEDIA_IMAGES`、`READ_MEDIA_VIDEO` 或 `AD_ID`。
- 2026-07-21 经用户明确授权，将上述唯一核验 AAB 上传并发布为 Internal `0.3.16-internal-1`。Play 预览确认 `19 (0.3.16)`、中英文 2 种版本说明，手机、平板、电视、Chromebook 与 Android XR 的不再支持设备数均为 0；Play Console 于 03:22（北京时间）显示该版本已面向内部测试人员发布，包含 1 个版本代码。本轮未推进 Alpha、开放测试或 Production，未部署 Worker、写 D1 或修改平台配置；Play 安装/覆盖更新验收仍待执行。
...
任何生产 Worker 部署都必须在上传前只读获取当前生产更新清单，并与拟部署源码中的 `versionCode` 比较；即使本次功能没有修改 `app_update.ts`，拟部署版本低于生产版本也必须失败停止，只有取得明确的更新清单回滚授权才能继续。仓库内测试只能证明清单与当前工作树的 `pubspec.yaml` 一致，不能发现旧功能分支相对生产环境的版本回退。
...
2026-07-21，经用户明确授权，在 Play Console 已独立确认 `0.3.17-internal-1` 全面可用后，从 `codex/play-0.3.17-candidate@63994f0` 对应 Worker 源码使用 Wrangler `4.107.1 deploy --keep-vars --strict` 部署更新清单；随后会员运营管理台从仍携带 `0.3.16 (19)` 清单的功能分支部署完整 Worker，意外覆盖生产清单。已安装 `versionCode=19` 的 App 因服务端清单不高于本地版本而在调用 Google Play 更新 API 前直接结束检查。

同日经用户重新授权，已从 `codex/restore-app-update-0.3.17@030d42e` 恢复：该提交以会员管理台最新基线叠加原 `0.3.17` 三个版本文件，部署使用 Wrangler `4.107.1 --keep-vars --strict`。部署前后 Worker 均为 `160/160`，远端无待迁移项、dry-run 通过；部署后中英文清单精确返回 `0.3.17 (20)` 和 4 条说明，Active 单一版本 100%，6 条既有鉴权探针均为 `401`，管理台入口继续由 Access 返回 `302`。未执行 D1 migration、未写 D1，也未修改 Secret、变量或 binding；精确恢复记录见私密配置记录 `CF-WORKER-20260721-APP-UPDATE-0.3.17-RESTORE-V2`。
...
2026-07-22，经用户明确授权，在 Internal `0.3.18 (21)` 验收通过后，把同一 AAB（`168435121` 字节，SHA-256 `37d0cea5663938ab8f6468269bb5a59a75b95c638039e810401daeacbba04c5f`，versionCode 21）从 Play 内容库推进到封闭式测试 - Alpha 轨道（Track ID `4700802545534122041`，177 个国家/地区），发布名 `0.3.18-closed-1`，未重新构建。中英文 release notes 各 4 条（与 Internal 一致），分阶段发布百分比 100%。当前状态为“已提交送审”（Play 发布概览显示“正在审核中的更改”，通常 7 天内完成），**审核通过前不算已发布**。

补记：2026-07-21 09:32 已把 `0.3.16 (19)` 从 Internal 推进到 Alpha `0.3.16-closed-1`，复用同一 Internal AAB（`187207830` 字节，SHA-256 `4C2E23654BB558BF94A920C2B6DD9CB98D1E935967B1B2D6C971C9AD3F25F614`，versionCode 19），面向 Alpha 测试人员全面发布。该推进当时未写独立 handoff，2026-07-22 经 main reviewer 审核发现后补记入私密台账 `handoffs/2026-07-21-play-0.3.16-alpha-promoted.md`（release notes 条数、分阶段百分比、配置记录 ID 与独立核对记录因补记而无证据，诚实标注为未记录）。

设备影响：0.3.16 → 0.3.18 Alpha 升级因 targetSdk 从 35 升到 36，手机不再支持 1 部（~0%），平板/车载各 +1 新支持；属 SDK 升级正常影响，非阻塞。

本次 Alpha 推进未重新构建产物、未推进开放测试或 Production、未执行 D1/Secret/变量/binding/RevenueCat/Google Cloud 写入。精确 Alpha Track 配置与发布 ID 只记录在本机私密台账。

2026-07-22，经用户明确授权，已完成 `0.3.19 (22)` 审计整改发版全流程（App + Worker + Alpha）。这是 main reviewer 在审核合并审计整改 18 commit（`4477ca3..4edff99`，含 Worker CSRF token 绑定、WorkoutController 单会话生命周期、MembershipApiClient 统一 15s 超时）后的发版推进。

版本号：`0.3.18+21` → `0.3.19+22`（pubspec + `workers/membership-api/src/app_update.ts` + `test/app-update.test.mjs` 同步，候选分支 `codex/play-0.3.19-candidate`，AAB 源提交 `14ef86d`，版本提交 `f8af3ad`）。

门禁（本会话亲自验证）：`flutter analyze` 0 issue、Flutter `715/715`、Worker `168/168`（含 0.3.19 清单期望值与 7 个 CSRF 新测试）、domain self-check `26/26`（回放硬基线 step0=5 / v3=5 / v4=3）、`git diff --check` 通过。

AAB 产物核验：`build/app/outputs/bundle/release/app-release.aab`，`168445115` 字节，SHA-256 `3c294f26e1a11c674482b7dfbd00692495fe1dd4b8eb1edf3f4bfd550ed71198`；`jarsigner -verify` 退出码 0 返回 `jar 已验证`（自签名上传证书的 PKIX/时间戳警告属正常）；上传证书 SHA-1 `5E:46:37:BD:25:99:5D:A2:24:8E:E4:F7:35:8C:EA:B4:71:3C:D3:B4` 与私密台账一致；`output-metadata.json` 确认 `applicationId=com.ugkexercise.ugk_exercise`、`versionCode=22`、`versionName=0.3.19`；Gradle 配置 `compileSdk=36 / minSdk=24 / targetSdk=36(flutter.targetSdkVersion)`；release 合并清单不含 `READ_MEDIA_IMAGES/READ_MEDIA_VIDEO/READ_EXTERNAL_STORAGE/WRITE_EXTERNAL_STORAGE/AD_ID`。

Worker 部署（两次，均 `wrangler 4.107.1 deploy --keep-vars`，未执行 D1 migration、未写 D1、未改 Secret/变量/binding）：
1. 从 `main@4edff99` 部署含管理台 CSRF token 绑定的 Worker（生产清单仍 0.3.18/21，仅让 `/admin/members/action` 与 `/admin/avatar-reports/action` 要求 actor 绑定 CSRF token）；Version ID `23fb5426-479a-48ef-b29b-905275600535`。
2. 从 `codex/play-0.3.19-candidate@14ef86d` 部署 0.3.19 中英文更新清单（拟部署 `22` > 生产 `21`，无回退）；Version ID `bfa46ac8-9137-4e22-b626-c5bb149724fb`。部署后 6 项只读核对全过（zh/en 返回 `0.3.19 (22)` 与 3 条说明，错误方法/平台返回 `405/400`，`/me` 未登录 `401`，`/admin` Access `302`）。

Play Console 独立核对：`0.3.19-internal-1` 于 2026-07-22 22:24（北京时间）面向内部测试人员发布，1 个版本代码；各设备类型不再支持数均为 0（手机 12,212 / 平板 6,553 / 电视 4 / Chromebook 69 / 车载 1 / Android XR 1，与 0.3.18 一致无减少）；新安装大小 41.5MB。Alpha `0.3.19-closed-1` 于 22:32 复用同一 AAB 从内容库推进，分阶段 100%，已提交送审（Play 显示“正在审核中的更改”，通常 7 天），**审核通过前不算已发布**。

审计整改审核报告：`docs/reviews/2026-07-22-audit-remediation-main-review-report.md`（18 commit 逐项只读复核，无 P0/P1 阻塞）。审计整改只实现了计划的 Milestone A（Task 1–3）；Task 4–12（数据完整性、死代码清理、JWT/X-Frame-Options 加固、性能测量、平台 P0）按设计未在本批落地，属后续批次。

本次发版未推进开放测试或 Production；真机训练/弱网冒烟（Controller race fix 与 API timeout 的真机端验证）仍待用户在 Internal 0.3.19 安装后反馈。精确 Worker Version ID、Alpha Track 配置与回滚目标只记录在本机私密台账。

2026-07-23，经用户明确授权，已完成 `0.3.20 (23)` 发版全流程（App Internal + Worker 清单 + Alpha 送审）。这是 main reviewer 审核合并三个分支（近距离 ready 阻断 `ce3bb29`、PR#13 三 bug 修复 `41d2e7a`、排行榜 UI 优化 `36ce274`）后的发版推进。

版本号：`0.3.19+22` → `0.3.20+23`（pubspec + `workers/membership-api/src/app_update.ts` + `test/app-update.test.mjs` 同步，候选分支 `codex/play-0.3.20-candidate`，AAB 源提交 `b8db7f5`）。

门禁（本会话亲自验证）：`flutter analyze` 0 issue、Flutter `730/730`、Worker `169/169`（含 0.3.20 清单期望值）、domain self-check `26/26`（回放硬基线 step0=5 / v3=5 / v4=3）、`git diff --check` 通过。

AAB 产物核验：`build/app/outputs/bundle/release/app-release.aab`，`168471142` 字节，SHA-256 `5c883d50ef20161f9cb4c7a23ba80193e75c1d4d6f033f491e5a05a2370a62e0`；`jarsigner -verify` 退出码 0 返回 `jar verified`；上传证书 SHA-1 `5E:46:37:BD:25:99:5D:A2:24:8E:E4:F7:35:8C:EA:B4:71:3C:D3:B4` 与私密台账一致；`output-metadata.json` 确认 `applicationId=com.ugkexercise.ugk_exercise`、`versionCode=23`、`versionName=0.3.20`；Gradle 配置 `compileSdk=36 / minSdk=24 / targetSdk=flutter.targetSdkVersion`；release 合并清单不含 `READ_MEDIA_IMAGES/VIDEO/EXTERNAL_STORAGE/AD_ID`，Release 不可调试。

Play Internal：AAB 上传 + 中英文 release notes 各 3 条 + 发布名 `0.3.20-internal-1`。Play Console 确认 `0.3.20-internal-1` 于 2026-07-23 12:18（北京时间）面向内部测试人员发布，1 个版本代码；各设备类型不再支持数均为 0（手机 12,212 / 平板 6,553 / 电视 4 / Chromebook 69 / 车载 1 / Android XR 1，与 0.3.19 一致无减少）。

Worker 部署：从 `codex/play-0.3.20-candidate@b8db7f5` 使用 `wrangler 4.107.1 deploy --keep-vars`，防回退核对（生产 22 < 拟部署 23），部署后 6 项只读核对全过（zh/en 返回 `0.3.20 (23)` 与 3 条说明，错误方法/平台返回 `405/400`，`/me` 未登录 `401`，`/admin` Access `302`）。本次部署仅更新 app_update 清单，本分支的 leaderboard breakdown 字段已在更早独立部署先行上线。未执行 D1 migration、未写 D1、未改 Secret/变量/binding；精确 Version ID 与回滚目标只记录在本机私密台账。

Alpha 推进：复用 Internal 同一 AAB（versionCode 23，未重新构建），发布名 `0.3.20-closed-1`，中英文 release notes 与 Internal 一致，177 个国家/地区，设备支持数不再支持均为 0。保存后进待送审草稿，发布概览提交送审。当前状态"正在审核中"（通常 7 天），**审核通过前不算已发布**。

本次发版未推进开放测试或 Production；真机覆盖更新验收（0.3.19→0.3.20 弹窗内容、商店跳转、覆盖更新、数据保留）仍待 Internal 测试人员安装后反馈。精确 Worker Version ID、Alpha Track 配置与回滚目标只记录在本机私密台账。
