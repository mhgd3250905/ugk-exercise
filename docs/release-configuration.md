...
| 内部测试新版本 | `0.3.6 (7)` USER REPORTED PUBLISHED | 用户报告 `0.3.6-internal-1` 已于 2026-07-15 09:14（北京时间）面向内部测试人员发布；已完成 Play 覆盖更新与非举报/屏蔽主链路验收 |
...
- 2026-07-15 后续用户验收：训练计数、语音、训练保存和记录页；中英文与浅色/深色主题；排行榜退出后重新加入及资料恢复；头像拍照、1:1 裁剪、取消、断网失败保护和联网重试；同账号退出后重新登录、会员恢复、排名及公开资料恢复均通过。举报与屏蔽未执行，留待补齐用户可操作的解除屏蔽能力后在独立会话验收。
