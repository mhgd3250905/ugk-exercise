...
| Google Play 测试轨道（最后核对 2026-07-17） | Internal `0.3.10 (13)` 已发布；Alpha `0.3.8-closed-1` 已发布 | 用户报告 `0.3.10-internal-1` 已通过审核并面向内部测试人员发布；Alpha 状态已由本会话独立核对 |
...
- 发布状态：用户报告发布名称 `0.3.10-internal-1` 已通过审核并面向内部测试人员发布；未提供 Console 精确发布时间。本会话未独立读取 Console，未推进 Alpha、开放测试或 Production；Play 安装验收仍待记录。
