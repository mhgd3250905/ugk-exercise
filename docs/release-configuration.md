...
| 会员单一权威对账 | WORKER PRODUCTION PASS / APP CANDIDATE BUILT | RevenueCat subscriber → Worker 权威裁决 → D1 可重建缓存已上线；包含 Flutter 强制对账语义的 `0.3.8 (9)` 候选已本地构建并核验，尚未上传或经 Play 安装验收 |
...
2026-07-16 `0.3.8 (9)` Google Play 候选（已本地构建核验，尚未上传）：
...
- 当前状态仅为本地已构建并核验；未上传 Google Play、未推进任何轨道、未送审，未执行购买、Worker/D1/Secret 或其他平台写入。购买后的强制对账与恢复路径仍须从 Play 内部测试安装此候选后做冒烟。
