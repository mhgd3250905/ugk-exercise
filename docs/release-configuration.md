...
- `0.3.14 (17)` 内部测试候选基于已同步的 `main@6774dc8`，候选分支为 `codex/play-0.3.14-candidate`，AAB 源提交为 `7f84e58ef3515b67254dbb19088084b5219e4220`。候选包含月卡 3 天与年卡 7 天试用卡片、精确 Offering 条款识别、无资格安全回退和 Google Play 订阅管理入口；不包含 Worker、D1 或 API 合同变更。
...
- 2026-07-20 上传前只读核对：Internal 最新版本仍为 `0.3.13-internal-1` / `versionCode=16`，状态“已面向内部测试人员发布”，且控制台允许创建新发布版本。`0.3.14 (17)` 当前状态仅为“已构建并核验，尚未上传”；计划发布名称为 `0.3.14-internal-1`，不得提前写成已上传或已发布。
