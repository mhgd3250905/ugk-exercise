...
| `0.3.12` 发布后独立审查 | ROUND 2 LOCAL PASS / INDEPENDENT REVERIFY PENDING / PLAY RUNTIME PENDING | 独立审查基线 `601d696`；主线程两轮修复提交 `2cef529`、`c92a00d` 已通过本地门禁，正等待同一独立线程第三轮复验；尚未构建或发布新的 AAB |
...
- `0.3.12` 发布后独立审查以 `601d696` 为只读候选；主线程第一轮 `2cef529` 修复首次加载与正常 restore 路径，第二轮 `c92a00d` 继续覆盖分页、批量刷新、身份刷新、旧快照回填，以及 restore 失败后由 refresh 接受会员结论的路径。第二轮本地门禁通过，独立第三轮复验待完成；这些修复均未进入已发布的 `versionCode=15` AAB。
...
- 审查线程只读核对需求完整性、逻辑正确性、边界情况、代码质量、测试覆盖和实际运行结果；候选基线为 `601d696`，两轮修复提交为 `2cef529`、`c92a00d`。
...
- 当前状态是“第二轮本地通过、同一独立审查线程第三轮复验待完成”，不得提前记为独立审查通过。
